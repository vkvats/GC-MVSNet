...
parser.add_argument('--device_id', type=int, default=0, help='select the gpu devide id to run the model')
...
#filter by gimupa
parser.add_argument('--fusibile_exe_path', type=str, default='/u/vkvats/tynamo/fusibile/fusibile')
...
def save_scene_depth(testlist):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            for filename, cam, img, depth_est, photometric_confidence, conf_1, conf_2 in zip(filenames, cams, imgs, outputs["depth"], outputs["photometric_confidence"],  outputs['stage1']["photometric_confidence"], outputs['stage2']["photometric_confidence"]):
                img = img[0]  #ref view
                cam = cam[0]  #ref cam
...
def pcd_filter_worker(scan):
    if args.testlist != "all":
        scan_id = int(scan[4:])
        save_name = 'normal_fusion/mvsnet{:0>3}_l3.ply'.format(scan_id)
    else:
...
if __name__ == '__main__':
    ...
    save_depth(testlist)
    # print(f"Skipping save_depth step and using previously estimated depth")
